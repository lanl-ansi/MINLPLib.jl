using JuMP, MathProgBase, Gurobi, CPLEX, Ipopt, POD
...
function multi4N(;verbose=false, solver=nothing,
				exprmode=1,
				convhull=false,
				uniform=-1,
				sos2=false, sos2_alter=false, facet=false,
				randomub=true,
				N=1,
				K=2,
				delta=4,
	...
	if solver == nothing
		if uniform > 0.0
			m = Model(solver=PODSolver(nlp_local_solver=IpoptSolver(print_level=0),
									   mip_solver=GurobiSolver(OutputFlag=1,Threads=16, BranchDir=1, VarBranch=2),
									   #mip_solver=CplexSolver(CPX_PARAM_SCRIND=1,CPX_PARAM_THREADS=8,CPX_PARAM_BRDIR=0,CPX_PARAM_VARSEL=0),
									   maxiter=1,
									   timeout=10080,
									   bilinear_convexhull=convhull,
...
function multi3N(;verbose=false, solver=nothing,
				  exprmode=1,
				  convhull=false, uniform=-1,
				  sos2=false, facet=false, sos2_alter=false,
				  randomub=true,
				  N=1,
				  K=2,
				  delta=4,
...
function multiKN(;verbose=false, solver=nothing,
				  exprmode=1,
				  convhull=false,
				  uniform=-1,
				  sos2=false, facet=false, sos2_alter=false,
	...
	if solver == nothing
		if uniform > 0.0
			m = Model(solver=PODSolver(nlp_local_solver=IpoptSolver(print_level=0),
									   mip_solver=GurobiSolver(OutputFlag=1,Threads=8, BranchDir=1, VarBranch=2),
									   maxiter=1,
									   timeout=10080,
									   bilinear_convexhull=convhull,
									   convexhull_use_sos2=sos2,
