...
# ----- Constraints ----- #
@constraint(m, e1, -x[1]+objvar == -7.25707902979242)
...
@NLconstraint(m, e8, (x[2]-x[14])*(x[2]-x[14])+(x[3]-x[15])*(x[3]-x[15]) >= 1.96)
...
@NLconstraint(m, e13, (x[4]-x[14])*(x[4]-x[14])+(x[5]-x[15])*(x[5]-x[15]) >= 1.96)
...
@NLconstraint(m, e20, (x[8]-x[14])*(x[8]-x[14])+(x[9]-x[15])*(x[9]-x[15]) >= 1.96)
@NLconstraint(m, e21, (x[10]-x[12])*(x[10]-x[12])+(x[11]-x[13])*(x[11]-x[13]) >= 1.0)
@NLconstraint(m, e22, (x[10]-x[14])*(x[10]-x[14])+(x[11]-x[15])*(x[11]-x[15]) >= 1.96)
@NLconstraint(m, e23, (x[12]-x[14])*(x[12]-x[14])+(x[13]-x[15])*(x[13]-x[15]) >= 1.96)
...
@constraint(m, e36, x[14]-x[16] <= -0.9)
@constraint(m, e37, x[15]-x[17] <= -0.9)
