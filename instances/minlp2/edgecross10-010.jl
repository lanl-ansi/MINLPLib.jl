...
@NLconstraint(m, e481, 2*b[5]-2*b[5]*b[50]+3*b[50]-2*b[5]*b[62]+b[62]+2*b[5]*b[74]-5*b[74]-2*b[5]*b[90]-b[90]-2*b[7]*b[60]+b[7]+2*b[60]-2*b[9]*b[60]+b[9]-2*b[13]*b[50]-2*b[13]*b[56]+b[56]+2*b[13]*b[74]+2*b[13]*b[86]-3*b[86]-2*b[15]*b[47]+b[47]-2*b[15]*b[50]+2*b[15]*b[68]-3*b[68]+2*b[15]*b[74]-2*b[25]*b[74]+2*b[25]*b[90]-2*b[26]*b[74]+2*b[26]*b[90]-2*b[28]*b[72]-b[28]+2*b[28]*b[75]-2*b[75]+2*b[28]*b[87]-b[87]+2*b[29]*b[63]-3*b[29]-2*b[63]+2*b[29]*b[69]-b[69]+2*b[29]*b[75]+2*b[33]*b[74]-2*b[33]+2*b[33]*b[86]+2*b[34]*b[68]-2*b[34]+2*b[34]*b[74]+2*b[37]*b[74]-2*b[37]+2*b[37]*b[86]+2*b[38]*b[68]-2*b[38]+2*b[38]*b[74]+2*b[43]*b[63]-3*b[43]+2*b[43]*b[66]-b[66]+2*b[43]*b[72]+objvar >= 12.0)
