...
# ----- Constraints ----- #
@NLconstraint(m, e1, -(300*x[1]-7*x[1]*x[1]-4*x[2]*x[2]+270*x[2]-6*x[3]*x[3]+460*x[3]-8*x[4]*x[4]+800*x[4]-12*x[5]*x[5]+740*x[5]-9*x[6]*x[6]+600*x[6]-14*x[7]*x[7]+540*x[7]-7*x[8]*x[8]+380*x[8]-13*x[9]*x[9]+300*x[9]-12*x[10]*x[10]+490*x[10]-8*x[11]*x[11]+380*x[11]-4*x[12]*x[12]+760*x[12]-7*x[13]*x[13]+430*x[13]-9*x[14]*x[14]+250*x[14]-16*x[15]*x[15]+390*x[15]-8*x[16]*x[16]+600*x[16]-4*x[17]*x[17]+210*x[17]-10*x[18]*x[18]+830*x[18]-21*x[19]*x[19]+470*x[19]-13*x[20]*x[20]+680*x[20]-17*x[21]*x[21]+360*x[21]-9*x[22]*x[22]+290*x[22]-8*x[23]*x[23]+400*x[23]-4*x[24]*x[24]+310*x[24])+objvar == 0.0)
