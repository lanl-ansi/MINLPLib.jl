...
# ----- Constraints ----- #
@constraint(m, e1, objvar+x[2]+x[3]-5*x[14]-10*x[15]+2*x[24]+x[25]-500*x[50]-600*x[51]-350*x[52]-400*x[53]+10*x[58]+5*x[59]+5*x[60]+5*x[61]-80*x[74]-130*x[75]-110*x[76]-120*x[77]-110*x[78]-130*x[79]-80*x[80]-90*x[81]+5*b[112]+4*b[113]+8*b[114]+7*b[115]+6*b[116]+9*b[117]+10*b[118]+9*b[119]+6*b[120]+10*b[121]+7*b[122]+7*b[123]+4*b[124]+3*b[125]+5*b[126]+6*b[127]+2*b[128]+5*b[129]+4*b[130]+7*b[131]+3*b[132]+9*b[133]+7*b[134]+2*b[135]+3*b[136]+b[137]+2*b[138]+6*b[139]+4*b[140]+8*b[141] == 0.0)
