...
setcategory(b[1], :Bin)
setlowerbound(x[13], 0.526315789473684)
setupperbound(x[13], 1.05263157894737)
setlowerbound(x[14], 0.961538461538462)
setupperbound(x[14], 2.11538461538462)
setlowerbound(x[15], 0.2)
setupperbound(x[15], 1.0)
setlowerbound(x[16], 0.8)
setupperbound(x[16], 0.8)
setlowerbound(x[17], 6.0)
setupperbound(x[17], 13.0)
setlowerbound(x[18], 6.0)
setupperbound(x[18], 13.0)
setlowerbound(x[19], 0.26)
setupperbound(x[19], 0.35)
setlowerbound(x[20], 4.9)
setupperbound(x[20], 5.5)
setlowerbound(x[21], 55.0)
setupperbound(x[21], 63.0)
setlowerbound(x[22], 0.296392099803303)
setupperbound(x[22], 0.404171045186323)
setlowerbound(x[23], 0.134723681728774)
setupperbound(x[23], 0.229030258938916)
setupperbound(x[24], 90.0)
setupperbound(x[25], 200.0)
setupperbound(x[26], 26.0)
setupperbound(x[27], 1.0)
setupperbound(x[28], 34.1)
...
@constraint(m, e6, -(-0.194075741585*x[13]^2-0.004843420334*x[14]^2+0.04736686635*x[15]^2+9.4029979e-5*x[13]*x[14]+0.011329651793*x[13]*x[15]-0.001017352942*x[14]*x[15]+0.382275988592*x[13]+0.019484588535*x[14]-0.077357069039*x[15])+x[19] == 0.140278656706)
...
@constraint(m, e12, -0.134723681728774*(0.010073140669*x[13]^2+0.011394190823*x[14]^2+0.052910213683*x[15]^2+0.000159410872*x[13]*x[14]+0.008036404292*x[13]*x[15]-0.003423392047*x[14]*x[15]+0.097124049148*x[13]+0.03829180344*x[14]+0.370440556286*x[15])+x[22] == 0.29587368369345)
@constraint(m, e13, -0.134723681728774*(0.46598008632*x[13]^2-0.00797004615*x[14]^2-0.01779288613*x[15]^2-0.01429434551*x[13]*x[14]-0.03832188467*x[13]*x[15]+0.00970510229*x[14]*x[15]-0.88981702163*x[13]+0.07730602595*x[14]+0.39988032723*x[15])+x[23] == 0.194162178290626)
...
@constraint(m, e18, b[1]*x[24] <= 0.0)
@constraint(m, e19, b[2]*x[24] >= 0.0)
@constraint(m, e20, b[2]*(-200+x[24]) <= 0.0)
@constraint(m, e21, b[3]*(-200+x[24]) >= 0.0)
...
@constraint(m, e32, b[7]*(-1+b[4]) >= 0.0)
@constraint(m, e33, b[9]*(-1+b[4]) >= 0.0)
