...
@constraint(m, e3, -0.36*x[2]-0.62*x[3]+0.99*x[4]+0.34*x[5]+0.35*x[6]-0.75*x[7]-0.99*x[8]+0.07*x[9]+0.82*x[10]-0.59*x[11]+0.56*x[12]+0.5*x[13]-0.7*x[14]-0.79*x[15]-0.91*x[16]+0.1*x[17]-0.43*x[18]+0.42*x[19]+0.29*x[20]+0.27*x[21]-0.34*x[22]+0.71*x[23]-0.46*x[24]-0.9*x[25]+0.87*x[26]+0.98*x[27]+0.28*x[28]+0.11*x[29]-0.74*x[30]+0.34*x[31]-0.07*x[32]-0.36*x[33]-0.78*x[34]+0.07*x[35]+0.42*x[36]-0.36*x[37]+x[38]-0.84*x[39]+0.64*x[40]-0.43*x[41]-0.8*x[42]+0.21*x[43]-0.12*x[44]-0.85*x[45]-0.46*x[46]+0.61*x[47]+0.22*x[48]+0.44*x[49]+0.32*x[50]+0.56*x[51]+0.86*x[52]+0.42*x[53]-0.32*x[54]-0.31*x[55]+0.91*x[56]+0.45*x[57]-0.26*x[58]-0.28*x[59]-0.71*x[60]+0.57*x[61] == 0.5)
@constraint(m, e4, -0.2*x[2]-0.74*x[3]+0.98*x[5]-0.65*x[6]+0.67*x[7]+0.22*x[8]+0.37*x[9]+0.65*x[10]+0.52*x[11]-0.74*x[12]-0.49*x[13]+0.46*x[14]+0.66*x[15]-0.1*x[16]-0.92*x[17]+0.47*x[18]+0.08*x[19]-0.79*x[20]-0.06*x[21]-0.72*x[22]+0.86*x[23]-0.71*x[24]-0.88*x[25]-0.8*x[26]-0.31*x[27]+0.91*x[28]+0.96*x[29]-0.22*x[30]+0.69*x[31]-0.81*x[32]-0.52*x[33]+0.82*x[34]-0.14*x[35]-0.14*x[36]-0.31*x[37]-0.72*x[38]-0.55*x[39]-0.77*x[40]-0.39*x[41]+0.92*x[42]-0.79*x[43]-0.92*x[44]+0.61*x[45]+0.01*x[46]-0.66*x[47]+0.63*x[48]-0.21*x[49]-0.23*x[50]+0.65*x[51]-0.93*x[52]-0.75*x[53]-0.09*x[54]+0.73*x[55]-0.72*x[56]-0.4*x[57]-0.65*x[58]-0.72*x[59]+0.21*x[60]+0.53*x[61] == -0.46)
@constraint(m, e5, 0.68*x[2]+0.41*x[3]+0.18*x[4]+0.34*x[5]-0.54*x[6]-0.04*x[7]+0.56*x[8]-0.09*x[9]-0.8*x[10]-0.75*x[11]-0.98*x[12]+0.08*x[13]+0.02*x[14]+0.91*x[15]-0.05*x[16]+0.18*x[17]-0.46*x[18]-0.49*x[19]+0.1*x[20]+0.48*x[21]+0.37*x[22]+0.14*x[23]+0.64*x[24]+0.9*x[25]+0.9*x[26]+0.41*x[27]+0.29*x[28]+0.28*x[29]+0.14*x[30]+0.65*x[31]+0.09*x[32]+0.1*x[33]-0.63*x[34]+0.48*x[35]-0.47*x[36]+0.36*x[37]-0.32*x[38]+0.39*x[39]+0.21*x[40]-0.83*x[41]-0.78*x[42]+0.91*x[43]-0.9*x[44]-0.29*x[45]+0.05*x[46]-0.72*x[47]-0.23*x[48]-0.55*x[49]+0.25*x[50]-0.69*x[51]-0.95*x[52]+0.24*x[53]-0.63*x[54]+0.56*x[55]-0.24*x[56]+0.51*x[57]+0.04*x[58]-0.36*x[59]+0.82*x[60]-x[61] == 0.59)
@constraint(m, e6, 0.15*x[2]+0.56*x[3]-0.14*x[4]-0.9*x[5]-0.28*x[6]-0.78*x[7]-0.68*x[8]+0.16*x[9]-0.07*x[10]-0.24*x[11]-0.83*x[12]-0.4*x[13]+0.01*x[14]+0.61*x[15]-0.71*x[16]-0.13*x[17]-0.65*x[18]-0.93*x[19]+0.3*x[20]-0.48*x[21]+0.55*x[22]-0.97*x[23]-0.42*x[24]-0.37*x[25]-0.61*x[26]+0.37*x[27]-0.57*x[28]+0.4*x[29]-0.23*x[30]+0.97*x[31]+0.08*x[32]-0.69*x[33]-0.07*x[34]+0.25*x[35]+0.33*x[37]+0.94*x[38]+0.31*x[39]+0.43*x[40]-0.57*x[41]+0.61*x[42]-0.32*x[43]-0.67*x[44]-0.88*x[45]-0.47*x[46]-0.57*x[47]-0.7*x[48]-0.2*x[49]+0.31*x[50]-0.34*x[51]+0.97*x[52]+0.82*x[53]+0.65*x[54]-0.13*x[55]-0.24*x[56]+0.68*x[57]+0.38*x[58]+0.94*x[59]-0.99*x[60]+0.47*x[61] == -0.21)
@constraint(m, e7, -0.81*x[2]+0.46*x[3]+0.86*x[4]-0.98*x[5]+0.11*x[6]-0.4*x[7]-0.33*x[8]-0.93*x[9]-0.59*x[10]-0.09*x[11]+0.3*x[12]-0.2*x[13]+0.84*x[14]+0.31*x[15]-0.38*x[16]-0.86*x[17]-0.14*x[18]+0.71*x[19]-0.38*x[20]-0.69*x[21]+0.12*x[22]+0.77*x[23]-0.67*x[24]+0.51*x[25]-0.22*x[26]+0.45*x[27]-0.1*x[28]-0.95*x[29]+0.38*x[30]+0.3*x[31]-0.22*x[32]-0.51*x[33]+0.8*x[34]+0.97*x[35]-0.86*x[36]+0.12*x[37]+0.19*x[38]+0.38*x[39]-0.95*x[40]-0.86*x[41]-0.9*x[42]+0.51*x[43]-0.25*x[44]+0.91*x[45]-0.93*x[46]-0.17*x[47]+0.87*x[48]+0.19*x[49]-0.88*x[50]+0.77*x[51]-0.18*x[52]+0.62*x[53]-0.16*x[54]-0.88*x[55]-0.6*x[56]-0.8*x[57]-0.64*x[58]+0.62*x[59]+0.73*x[60]+0.18*x[61] == -0.33)
@constraint(m, e8, -0.37*x[2]-0.18*x[3]+0.67*x[4]+0.62*x[5]+0.15*x[6]-0.95*x[7]-0.86*x[8]-0.92*x[9]+0.6*x[10]+0.65*x[11]+0.11*x[12]+0.21*x[13]-0.7*x[14]+0.14*x[15]-0.69*x[16]+0.24*x[17]-0.53*x[18]+0.07*x[19]-0.14*x[20]+0.11*x[21]+0.56*x[22]-0.31*x[23]+0.54*x[24]+0.29*x[25]-0.7*x[26]+0.46*x[27]-0.48*x[28]-0.01*x[29]+0.28*x[30]+0.28*x[31]+0.52*x[32]+0.66*x[33]+0.71*x[34]-0.12*x[35]+0.1*x[36]-0.79*x[37]+x[38]+0.9*x[39]+0.84*x[40]-0.68*x[41]-0.62*x[42]+0.73*x[43]+0.44*x[44]+0.02*x[45]+0.99*x[46]+0.16*x[47]-0.54*x[48]+0.79*x[49]-0.57*x[50]-0.72*x[51]+0.66*x[52]+0.1*x[53]-0.05*x[54]-0.45*x[55]+0.16*x[56]-0.94*x[57]+0.5*x[58]-0.08*x[59]-0.53*x[60]-0.97*x[61] == -0.23)
