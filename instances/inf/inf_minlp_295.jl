...
@constraint(m, e175, x[10]-3.75*b[32]-3.75*b[39]+1.75*b[46] <= -1.75)
@constraint(m, e176, x[11]-7.3*b[33]-7.3*b[40]+4.05*b[47] <= -4.75)
@constraint(m, e177, x[12]-7*b[34]-7*b[41]+3.75*b[48] <= -5.25)
@constraint(m, e178, x[13]-3.5*b[35]-3.5*b[42]+1.75*b[49] <= -4.75)
@constraint(m, e179, x[14]-3.3*b[36]-3.3*b[43]+1.8*b[50] <= -1.25)
...
@NLconstraint(m, e187, x[24]*b[44]+x[22]*(1-b[30]-b[37])+x[26]*(b[30]+b[37]-b[44])+x[8] <= 3.25)
@NLconstraint(m, e188, x[25]*b[45]+x[23]*(1-b[31]-b[38])+x[27]*(b[31]+b[38]-b[45])+x[9] <= 3.25)
