...
@constraint(m, e226, x[4]+4.76*b[39]+4.56*b[47]-4.56*b[55]+4.56*b[63] <= 11.36)
@constraint(m, e227, x[5]+5.35*b[40]+5.25*b[48]-5.25*b[56]+5.25*b[64] <= 11.45)
@constraint(m, e228, x[6]+5.34*b[41]+5.24*b[49]-5.24*b[57]+5.24*b[65] <= 11.44)
@constraint(m, e229, x[7]+7.21*b[42]+3.86*b[50]-3.86*b[58]+3.86*b[66] <= 10.96)
@constraint(m, e230, x[8]+5.6*b[43]+3.75*b[51]-3.75*b[59]+3.75*b[67] <= 11.35)
...
@NLconstraint(m, e241, x[25]*b[36]+x[28]*(b[44]-b[52]+b[60])+x[31]*(1-b[36]-b[44]+b[52]-b[60])+x[1] <= 12.0)
@NLconstraint(m, e242, x[26]*b[37]+x[29]*(b[45]-b[53]+b[61])+x[32]*(1-b[37]-b[45]+b[53]-b[61])+x[2] <= 12.0)
@NLconstraint(m, e243, x[27]*b[38]+x[30]*(b[46]-b[54]+b[62])+x[33]*(1-b[38]-b[46]+b[54]-b[62])+x[3] <= 12.0)
...
@NLconstraint(m, e315, -x[26]*x[29]*x[32] == -6.0)
@NLconstraint(m, e316, -x[27]*x[30]*x[33] == -6.0)
@constraint(m, e317, x[25] <= 5.5)
@constraint(m, e318, x[26] <= 5.0)
@constraint(m, e319, x[27] <= 5.5)
@constraint(m, e320, x[25] >= 4.125)
@constraint(m, e321, x[26] >= 2.375)
@constraint(m, e322, x[27] >= 4.0)
@constraint(m, e323, x[28] <= 2.375)
@constraint(m, e324, x[29] <= 4.125)
@constraint(m, e325, x[30] <= 6.0)
@constraint(m, e326, x[28] >= 2.125)
@constraint(m, e327, x[29] >= 4.25)
@constraint(m, e328, x[30] >= 6.0)
@constraint(m, e329, x[31] <= 0.375)
@constraint(m, e330, x[32] <= 0.25)
@constraint(m, e331, x[33] <= 0.5)
@constraint(m, e332, x[31] >= 0.375)
@constraint(m, e333, x[32] >= 0.25)
@constraint(m, e334, x[33] >= 0.5)
