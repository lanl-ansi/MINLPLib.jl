...
@constraint(m, e226, x[4]+4.76*b[39]+4.56*b[47]-4.56*b[55]+4.56*b[63] <= 4.36)
@constraint(m, e227, x[5]+5.35*b[40]+5.25*b[48]-5.25*b[56]+5.25*b[64] <= 4.45)
@constraint(m, e228, x[6]+5.34*b[41]+5.24*b[49]-5.24*b[57]+5.24*b[65] <= 4.44)
@constraint(m, e229, x[7]+7.21*b[42]+3.86*b[50]-3.86*b[58]+3.86*b[66] <= 3.96)
@constraint(m, e230, x[8]+5.6*b[43]+3.75*b[51]-3.75*b[59]+3.75*b[67] <= 4.35)
...
@constraint(m, e236, x[20]-x[34]+0.2*b[47]-4.56*b[63] <= -5.2)
@constraint(m, e237, x[21]-x[34]+0.100000000000001*b[48]-5.25*b[64] <= -5.8)
@constraint(m, e238, x[22]-x[34]+0.100000000000001*b[49]-5.24*b[65] <= -5.8)
@constraint(m, e239, x[23]-x[34]+3.35*b[50]-3.86*b[66] <= -4.9)
@constraint(m, e240, x[24]-x[34]+1.85*b[51]-3.75*b[67] <= -4.4)
@NLconstraint(m, e241, x[25]*b[36]+x[28]*(b[44]-b[52]+b[60])+x[31]*(1-b[36]-b[44]+b[52]-b[60])+x[1] <= 5.0)
@NLconstraint(m, e242, x[26]*b[37]+x[29]*(b[45]-b[53]+b[61])+x[32]*(1-b[37]-b[45]+b[53]-b[61])+x[2] <= 5.0)
@NLconstraint(m, e243, x[27]*b[38]+x[30]*(b[46]-b[54]+b[62])+x[33]*(1-b[38]-b[46]+b[54]-b[62])+x[3] <= 5.0)
