...
@NLconstraint(m, e21, x[2]*x[11]+x[12]*x[21]-0.960170286650366*x[3]-0.279415498198926*x[13] == 0.0)
@NLconstraint(m, e22, x[3]*x[10]+x[13]*x[20]-0.843853958732492*x[5]-0.536572918000435*x[15] == 0.0)
@NLconstraint(m, e23, x[4]*x[9]+x[14]*x[19]-0.66031670824408*x[7]-0.750987246771676*x[17] == 0.0)
@NLconstraint(m, e24, x[5]*x[8]+x[15]*x[18]-0.424179007336997*x[9]-0.905578362006624*x[19] == 0.0)
@NLconstraint(m, e25, x[6]*x[7]+x[16]*x[17]-0.154251449887584*x[11]-0.988031624092862*x[21] == 0.0)
@NLconstraint(m, e26, x[6]*x[7]+x[16]*x[17]+0.127963689627405*x[11]+0.991778853443116*x[21] == 0.0)
@NLconstraint(m, e27, x[5]*x[8]+x[15]*x[18]+0.399985314988351*x[9]+0.916521547915634*x[19] == 0.0)
@NLconstraint(m, e28, x[4]*x[9]+x[14]*x[19]+0.6401443394692*x[7]+0.768254661323667*x[17] == 0.0)
@NLconstraint(m, e29, x[3]*x[10]+x[13]*x[20]+0.82930983286315*x[5]+0.558789048851616*x[15] == 0.0)
@NLconstraint(m, e30, x[2]*x[11]+x[12]*x[21]+0.952412980415156*x[3]+0.304810621102217*x[13] == 0.0)
@NLconstraint(m, e31, x[11]*x[12]-x[2]*x[21]+0.279415498198926*x[3]-5.7610217199022*x[13] == 0.0)
@NLconstraint(m, e32, x[10]*x[13]-x[3]*x[20]-5.06312375239495*x[13]+0.536572918000435*x[5] == 0.0)
@NLconstraint(m, e33, x[9]*x[14]-x[4]*x[19]+0.750987246771676*x[7]-3.96190024946448*x[13] == 0.0)
@NLconstraint(m, e34, x[8]*x[15]-x[5]*x[18]+0.905578362006624*x[9]-2.54507404402198*x[13] == 0.0)
@NLconstraint(m, e35, x[7]*x[16]-x[6]*x[17]+0.988031624092862*x[11]-0.925508699325504*x[13] == 0.0)
@NLconstraint(m, e36, x[6]*x[17]-x[7]*x[16]+0.991778853443116*x[11]+0.767782137764428*x[13] == 0.0)
@NLconstraint(m, e37, x[5]*x[18]-x[8]*x[15]+0.916521547915634*x[9]+2.39991188993011*x[13] == 0.0)
@NLconstraint(m, e38, x[4]*x[19]-x[9]*x[14]+0.768254661323667*x[7]+3.8408660368152*x[13] == 0.0)
@NLconstraint(m, e39, x[3]*x[20]-x[10]*x[13]+4.9758589971789*x[13]+0.558789048851616*x[5] == 0.0)
@NLconstraint(m, e40, x[2]*x[21]-x[11]*x[12]+0.304810621102217*x[3]+5.71447788249094*x[13] == 0.0)
