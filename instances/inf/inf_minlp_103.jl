...
@constraint(m, e170, x[3]+7.7275*b[32]+4.24*b[39]-4.24*b[46]+4.24*b[53] <= 4.54)
@constraint(m, e171, x[4]+4.3*b[33]+4.2*b[40]-4.2*b[47]+4.2*b[54] <= 3.9)
@constraint(m, e172, x[5]+7.52*b[34]+3.72*b[41]-3.72*b[48]+3.72*b[55] <= 4.02)
@constraint(m, e173, x[6]+7.44*b[35]+3.99*b[42]-3.99*b[49]+3.99*b[56] <= 4.19)
@constraint(m, e174, x[7]+6.695*b[36]+4.13*b[43]-4.13*b[50]+4.13*b[57] <= 4.32)
...
@constraint(m, e180, x[17]-x[28]+3.4875*b[39]-4.24*b[53] <= -4.7)
@constraint(m, e181, x[18]-x[28]+0.100000000000001*b[40]-4.2*b[54] <= -5.3)
@constraint(m, e182, x[19]-x[28]+3.8*b[41]-3.72*b[55] <= -4.7)
@constraint(m, e183, x[20]-x[28]+3.45*b[42]-3.99*b[56] <= -4.8)
@constraint(m, e184, x[21]-x[28]+2.565*b[43]-4.13*b[57] <= -4.81)
@NLconstraint(m, e185, x[22]*b[30]+x[24]*(b[37]-b[44]+b[51])+x[26]*(1-b[30]-b[37]+b[44]-b[51])+x[1] <= 5.0)
@NLconstraint(m, e186, x[23]*b[31]+x[25]*(b[38]-b[45]+b[52])+x[27]*(1-b[31]-b[38]+b[45]-b[52])+x[2] <= 5.0)
