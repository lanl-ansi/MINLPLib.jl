...
@constraint(m, e226, x[3]+6.5125*b[35]+4.1*b[43]-4.1*b[51]+4.1*b[59] <= 4.2)
@constraint(m, e227, x[4]+4.78*b[36]+1.78*b[44]-1.78*b[52]+1.78*b[60] <= 4.18)
@constraint(m, e228, x[5]+5*b[37]+4.5*b[45]-4.5*b[53]+4.5*b[61] <= 4.0)
@constraint(m, e229, x[6]+7.25*b[38]+3.9*b[46]-3.9*b[54]+3.9*b[62] <= 4.0)
@constraint(m, e230, x[7]+5.01*b[39]+4.81*b[47]-4.81*b[55]+4.81*b[63] <= 4.61)
@constraint(m, e231, x[8]+5.6*b[40]+3.75*b[48]-3.75*b[56]+3.75*b[64] <= 4.35)
...
@constraint(m, e238, x[19]-x[31]+2.4125*b[43]-4.1*b[59] <= -4.9)
@constraint(m, e239, x[20]-x[31]+3*b[44]-1.78*b[60] <= -2.6)
@constraint(m, e240, x[21]-x[31]+0.5*b[45]-4.5*b[61] <= -5.5)
@constraint(m, e241, x[22]-x[31]+3.35*b[46]-3.9*b[62] <= -4.9)
@constraint(m, e242, x[23]-x[31]+0.2*b[47]-4.81*b[63] <= -5.2)
@constraint(m, e243, x[24]-x[31]+1.85*b[48]-3.75*b[64] <= -4.4)
@NLconstraint(m, e244, x[25]*b[33]+x[27]*(b[41]-b[49]+b[57])+x[29]*(1-b[33]-b[41]+b[49]-b[57])+x[1] <= 5.0)
@NLconstraint(m, e245, x[26]*b[34]+x[28]*(b[42]-b[50]+b[58])+x[30]*(1-b[34]-b[42]+b[50]-b[58])+x[2] <= 5.0)
