...
@constraint(m, e226, x[4]+5.55*b[39]+4.55*b[47]-4.55*b[55]+4.55*b[63] <= 17.8)
@constraint(m, e227, x[5]+4.9*b[40]+4.8*b[48]-4.8*b[56]+4.8*b[64] <= 18.0)
@constraint(m, e228, x[6]+5.6*b[41]+3.75*b[49]-3.75*b[57]+3.75*b[65] <= 18.35)
@constraint(m, e229, x[7]+5.25*b[42]+5.05*b[50]-5.05*b[58]+5.05*b[66] <= 18.45)
@constraint(m, e230, x[8]+5.25*b[43]+5.2*b[51]-5.2*b[59]+5.2*b[67] <= 18.4)
...
@NLconstraint(m, e241, x[25]*b[36]+x[28]*(b[44]-b[52]+b[60])+x[31]*(1-b[36]-b[44]+b[52]-b[60])+x[1] <= 19.0)
@NLconstraint(m, e242, x[26]*b[37]+x[29]*(b[45]-b[53]+b[61])+x[32]*(1-b[37]-b[45]+b[53]-b[61])+x[2] <= 19.0)
@NLconstraint(m, e243, x[27]*b[38]+x[30]*(b[46]-b[54]+b[62])+x[33]*(1-b[38]-b[46]+b[54]-b[62])+x[3] <= 19.0)
...
@NLconstraint(m, e315, -x[26]*x[29]*x[32] == -11.0)
