...
@constraint(m, e170, x[3]+5.25*b[32]+5.05*b[39]-5.05*b[46]+5.05*b[53] <= 4.35)
@constraint(m, e171, x[4]+7.1*b[33]+3.6*b[40]-3.6*b[47]+3.6*b[54] <= 4.0)
@constraint(m, e172, x[5]+5.17*b[34]+3.67*b[41]-3.67*b[48]+3.67*b[55] <= 4.17)
@constraint(m, e173, x[6]+4.57*b[35]+4.47*b[42]-4.47*b[49]+4.47*b[56] <= 3.77)
@constraint(m, e174, x[7]+7.3875*b[36]+4.02*b[43]-4.02*b[50]+4.02*b[57] <= 4.2)
...
@constraint(m, e180, x[17]-x[28]+0.2*b[39]-5.05*b[53] <= -5.7)
@constraint(m, e181, x[18]-x[28]+3.5*b[40]-3.6*b[54] <= -4.6)
@constraint(m, e182, x[19]-x[28]+1.5*b[41]-3.67*b[55] <= -4.5)
@constraint(m, e183, x[20]-x[28]+0.0999999999999996*b[42]-4.47*b[56] <= -5.7)
@constraint(m, e184, x[21]-x[28]+3.3675*b[43]-4.02*b[57] <= -4.82)
