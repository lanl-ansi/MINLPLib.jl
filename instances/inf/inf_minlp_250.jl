...
@constraint(m, e170, x[4]+7.1*b[36]+3.6*b[43]-3.6*b[50]+3.6*b[57] <= 4.0)
@constraint(m, e171, x[5]+5.11*b[37]+4.86*b[44]-4.86*b[51]+4.86*b[58] <= 4.3)
@constraint(m, e172, x[6]+7.325*b[38]+4*b[45]-4*b[52]+4*b[59] <= 4.2)
@constraint(m, e173, x[7]+6.7975*b[39]+4.16*b[46]-4.16*b[53]+4.16*b[60] <= 4.36)
...
@constraint(m, e178, x[18]-x[31]+3.5*b[43]-3.6*b[57] <= -4.6)
@constraint(m, e179, x[19]-x[31]+0.25*b[44]-4.86*b[58] <= -5.56)
@constraint(m, e180, x[20]-x[31]+3.325*b[45]-4*b[59] <= -4.8)
@constraint(m, e181, x[21]-x[31]+2.6375*b[46]-4.16*b[60] <= -4.8)
@NLconstraint(m, e182, x[22]*b[33]+x[25]*(b[40]-b[47]+b[54])+x[28]*(1-b[33]-b[40]+b[47]-b[54])+x[1] <= 5.0)
@NLconstraint(m, e183, x[23]*b[34]+x[26]*(b[41]-b[48]+b[55])+x[29]*(1-b[34]-b[41]+b[48]-b[55])+x[2] <= 5.0)
@NLconstraint(m, e184, x[24]*b[35]+x[27]*(b[42]-b[49]+b[56])+x[30]*(1-b[35]-b[42]+b[49]-b[56])+x[3] <= 5.0)
...
@NLconstraint(m, e188, x[28]*b[54]+x[25]*(1-b[40]-b[54])+x[22]*b[40]+x[15]-x[31] <= 0.0)
@NLconstraint(m, e189, x[29]*b[55]+x[26]*(1-b[41]-b[55])+x[23]*b[41]+x[16]-x[31] <= 0.0)
@NLconstraint(m, e190, x[30]*b[56]+x[27]*(1-b[42]-b[56])+x[24]*b[42]+x[17]-x[31] <= 0.0)
