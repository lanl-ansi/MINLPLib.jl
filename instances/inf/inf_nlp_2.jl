...
@NLconstraint(m, e21, x[2]*x[11]+x[12]*x[21]+0.989992496600445*x[3]+0.141120008059867*x[13] == 0.0)
@NLconstraint(m, e22, x[3]*x[10]+x[13]*x[20]-0.960170286650366*x[5]-0.279415498198926*x[15] == 0.0)
@NLconstraint(m, e23, x[4]*x[9]+x[14]*x[19]+0.911130261884677*x[7]+0.412118485241757*x[17] == 0.0)
@NLconstraint(m, e24, x[5]*x[8]+x[15]*x[18]-0.843853958732492*x[9]-0.536572918000435*x[19] == 0.0)
@NLconstraint(m, e25, x[6]*x[7]+x[16]*x[17]+0.759687912858821*x[11]+0.650287840157117*x[21] == 0.0)
@NLconstraint(m, e26, x[6]*x[7]+x[16]*x[17]-0.66031670824408*x[11]+0.750987246771676*x[21] == 0.0)
@NLconstraint(m, e27, x[5]*x[8]+x[15]*x[18]+0.547729260224268*x[9]-0.836655638536056*x[19] == 0.0)
@NLconstraint(m, e28, x[4]*x[9]+x[14]*x[19]-0.424179007336997*x[7]+0.905578362006624*x[17] == 0.0)
@NLconstraint(m, e29, x[3]*x[10]+x[13]*x[20]+0.292138808733836*x[5]-0.956375928404503*x[15] == 0.0)
@NLconstraint(m, e30, x[2]*x[11]+x[12]*x[21]-0.154251449887584*x[3]+0.988031624092862*x[13] == 0.0)
@NLconstraint(m, e31, x[11]*x[12]-x[2]*x[21]-0.141120008059867*x[3]+2.96997748980134*x[13] == 0.0)
@NLconstraint(m, e32, x[10]*x[13]-x[3]*x[20]-2.8805108599511*x[13]+0.279415498198926*x[5] == 0.0)
@NLconstraint(m, e33, x[9]*x[14]-x[4]*x[19]-0.412118485241757*x[7]+2.73339078565403*x[13] == 0.0)
@NLconstraint(m, e34, x[8]*x[15]-x[5]*x[18]+0.536572918000435*x[9]-2.53156187619748*x[13] == 0.0)
@NLconstraint(m, e35, x[7]*x[16]-x[6]*x[17]-0.650287840157117*x[11]+2.27906373857646*x[13] == 0.0)
@NLconstraint(m, e36, x[6]*x[17]-x[7]*x[16]+0.750987246771676*x[11]-1.98095012473224*x[13] == 0.0)
@NLconstraint(m, e37, x[5]*x[18]-x[8]*x[15]-0.836655638536056*x[9]+1.64318778067281*x[13] == 0.0)
@NLconstraint(m, e38, x[4]*x[19]-x[9]*x[14]+0.905578362006624*x[7]-1.27253702201099*x[13] == 0.0)
@NLconstraint(m, e39, x[3]*x[20]-x[10]*x[13]+0.876416426201509*x[13]-0.956375928404503*x[5] == 0.0)
@NLconstraint(m, e40, x[2]*x[21]-x[11]*x[12]+0.988031624092862*x[3]-0.462754349662752*x[13] == 0.0)
