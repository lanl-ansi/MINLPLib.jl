...
@NLconstraint(m, e10, -(5* (x[1])^2+10* (x[2])^2+500* (x[3])^2+5* (x[4])^2+20* (x[5])^2+5* (x[6])^2+10* (x[7])^2+35* (x[8])^2+95* (x[9])^2+70* (x[10])^2+20* (x[11])^2+10* (x[12])^2+35* (x[13])^2+10* (x[14])^2+30* (x[15])^2) >= -860.0)
@constraint(m, e11, 486*x[1]+640*x[2]+758*x[3]+776*x[4]+477*x[5]+707*x[6]+175*x[7]+619*x[8]+627*x[9]+614*x[10]+475*x[11]+377*x[12]+524*x[13]+468*x[14]+529*x[15]+objvar == 0.0)
