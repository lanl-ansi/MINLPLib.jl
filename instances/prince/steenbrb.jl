...
@NLconstraint(m, e109, -0.001*(0.35* (x[37]+x[55]+x[73]+x[91]+x[109]+x[127]+x[145]+x[163]+x[181]+x[199]+x[217]+x[235])^2/(x[1]*x[1])+0.35*x[37]+0.35*x[55]+0.35*x[73]+0.35*x[91]+0.35*x[109]+0.35*x[127]+0.35*x[145]+0.35*x[163]+0.35*x[181]+0.35*x[199]+0.35*x[217]+0.35*x[235]+0.35* (x[253]+x[271]+x[289]+x[307]+x[325]+x[343]+x[361]+x[379]+x[397]+x[415]+x[433]+x[451])^2/(x[19]*x[19])+0.35*x[253]+0.35*x[271]+0.35*x[289]+0.35*x[307]+0.35*x[325]+0.35*x[343]+0.35*x[361]+0.35*x[379]+0.35*x[397]+0.35*x[415]+0.35*x[433]+0.35*x[451]+0.35*sqrt(x[1])+0.35*sqrt(x[19])+0.4* (x[38]+x[56]+x[74]+x[92]+x[110]+x[128]+x[146]+x[164]+x[182]+x[200]+x[218]+x[236])^2/(x[2]*x[2])+0.4*x[38]+0.4*x[56]+0.4*x[74]+0.4*x[92]+0.4*x[110]+0.4*x[128]+0.4*x[146]+0.4*x[164]+0.4*x[182]+0.4*x[200]+0.4*x[218]+0.4*x[236]+0.4* (x[254]+x[272]+x[290]+x[308]+x[326]+x[344]+x[362]+x[380]+x[398]+x[416]+x[434]+x[452])^2/(x[20]*x[20])+0.4*x[254]+0.4*x[272]+0.4*x[290]+0.4*x[308]+0.4*x[326]+0.4*x[344]+0.4*x[362]+0.4*x[380]+0.4*x[398]+0.4*x[416]+0.4*x[434]+0.4*x[452]+0.4*sqrt(x[2])+0.4*sqrt(x[20])+0.3* (x[39]+x[57]+x[75]+x[93]+x[111]+x[129]+x[147]+x[165]+x[183]+x[201]+x[219]+x[237])^2/(x[3]*x[3])+0.3*x[39]+0.3*x[57]+0.3*x[75]+0.3*x[93]+0.3*x[111]+0.3*x[129]+0.3*x[147]+0.3*x[165]+0.3*x[183]+0.3*x[201]+0.3*x[219]+0.3*x[237]+0.3* (x[255]+x[273]+x[291]+x[309]+x[327]+x[345]+x[363]+x[381]+x[399]+x[417]+x[435]+x[453])^2/(x[21]*x[21])+0.3*x[255]+0.3*x[273]+0.3*x[291]+0.3*x[309]+0.3*x[327]+0.3*x[345]+0.3*x[363]+0.3*x[381]+0.3*x[399]+0.3*x[417]+0.3*x[435]+0.3*x[453]+0.3*sqrt(x[3])+0.3*sqrt(x[21])+ (x[40]+x[58]+x[76]+x[94]+x[112]+x[130]+x[148]+x[166]+x[184]+x[202]+x[220]+x[238])^2/(x[4]*x[4])+x[40]+x[58]+x[76]+x[94]+x[112]+x[130]+x[148]+x[166]+x[184]+x[202]+x[220]+x[238]+ (x[256]+x[274]+x[292]+x[310]+x[328]+x[346]+x[364]+x[382]+x[400]+x[418]+x[436]+x[454])^2/(x[22]*x[22])+x[256]+x[274]+x[292]+x[310]+x[328]+x[346]+x[364]+x[382]+x[400]+x[418]+x[436]+x[454]+sqrt(x[4])+sqrt(x[22])+0.15* (x[41]+x[59]+x[77]+x[95]+x[113]+x[131]+x[149]+x[167]+x[185]+x[203]+x[221]+x[239])^2/(x[5]*x[5])+0.15*x[41]+0.15*x[59]+0.15*x[77]+0.15*x[95]+0.15*x[113]+0.15*x[131]+0.15*x[149]+0.15*x[167]+0.15*x[185]+0.15*x[203]+0.15*x[221]+0.15*x[239]+0.15* (x[257]+x[275]+x[293]+x[311]+x[329]+x[347]+x[365]+x[383]+x[401]+x[419]+x[437]+x[455])^2/(x[23]*x[23])+0.15*x[257]+0.15*x[275]+0.15*x[293]+0.15*x[311]+0.15*x[329]+0.15*x[347]+0.15*x[365]+0.15*x[383]+0.15*x[401]+0.15*x[419]+0.15*x[437]+0.15*x[455]+0.15*sqrt(x[5])+0.15*sqrt(x[23])+0.55* (x[42]+x[60]+x[78]+x[96]+x[114]+x[132]+x[150]+x[168]+x[186]+x[204]+x[222]+x[240])^2/(x[6]*x[6])+0.55*x[42]+0.55*x[60]+0.55*x[78]+0.55*x[96]+0.55*x[114]+0.55*x[132]+0.55*x[150]+0.55*x[168]+0.55*x[186]+0.55*x[204]+0.55*x[222]+0.55*x[240]+0.55* (x[258]+x[276]+x[294]+x[312]+x[330]+x[348]+x[366]+x[384]+x[402]+x[420]+x[438]+x[456])^2/(x[24]*x[24])+0.55*x[258]+0.55*x[276]+0.55*x[294]+0.55*x[312]+0.55*x[330]+0.55*x[348]+0.55*x[366]+0.55*x[384]+0.55*x[402]+0.55*x[420]+0.55*x[438]+0.55*x[456]+0.55*sqrt(x[6])+0.55*sqrt(x[24])+ (x[43]+x[61]+x[79]+x[97]+x[115]+x[133]+x[151]+x[169]+x[187]+x[205]+x[223]+x[241])^2/(x[7]*x[7])+x[43]+x[61]+x[79]+x[97]+x[115]+x[133]+x[151]+x[169]+x[187]+x[205]+x[223]+x[241]+ (x[259]+x[277]+x[295]+x[313]+x[331]+x[349]+x[367]+x[385]+x[403]+x[421]+x[439]+x[457])^2/(x[25]*x[25])+x[259]+x[277]+x[295]+x[313]+x[331]+x[349]+x[367]+x[385]+x[403]+x[421]+x[439]+x[457]+sqrt(x[7])+sqrt(x[25])+0.25* (x[44]+x[62]+x[80]+x[98]+x[116]+x[134]+x[152]+x[170]+x[188]+x[206]+x[224]+x[242])^2/(x[8]*x[8])+0.25*x[44]+0.25*x[62]+0.25*x[80]+0.25*x[98]+0.25*x[116]+0.25*x[134]+0.25*x[152]+0.25*x[170]+0.25*x[188]+0.25*x[206]+0.25*x[224]+0.25*x[242]+0.25* (x[260]+x[278]+x[296]+x[314]+x[332]+x[350]+x[368]+x[386]+x[404]+x[422]+x[440]+x[458])^2/(x[26]*x[26])+0.25*x[260]+0.25*x[278]+0.25*x[296]+0.25*x[314]+0.25*x[332]+0.25*x[350]+0.25*x[368]+0.25*x[386]+0.25*x[404]+0.25*x[422]+0.25*x[440]+0.25*x[458]+0.25*sqrt(x[8])+0.25*sqrt(x[26])+0.6* (x[45]+x[63]+x[81]+x[99]+x[117]+x[135]+x[153]+x[171]+x[189]+x[207]+x[225]+x[243])^2/(x[9]*x[9])+0.6*x[45]+0.6*x[63]+0.6*x[81]+0.6*x[99]+0.6*x[117]+0.6*x[135]+0.6*x[153]+0.6*x[171]+0.6*x[189]+0.6*x[207]+0.6*x[225]+0.6*x[243]+0.6* (x[261]+x[279]+x[297]+x[315]+x[333]+x[351]+x[369]+x[387]+x[405]+x[423]+x[441]+x[459])^2/(x[27]*x[27])+0.6*x[261]+0.6*x[279]+0.6*x[297]+0.6*x[315]+0.6*x[333]+0.6*x[351]+0.6*x[369]+0.6*x[387]+0.6*x[405]+0.6*x[423]+0.6*x[441]+0.6*x[459]+0.6*sqrt(x[9])+0.6*sqrt(x[27])+0.35* (x[46]+x[64]+x[82]+x[100]+x[118]+x[136]+x[154]+x[172]+x[190]+x[208]+x[226]+x[244])^2/(x[10]*x[10])+0.35*x[46]+0.35*x[64]+0.35*x[82]+0.35*x[100]+0.35*x[118]+0.35*x[136]+0.35*x[154]+0.35*x[172]+0.35*x[190]+0.35*x[208]+0.35*x[226]+0.35*x[244]+0.35* (x[262]+x[280]+x[298]+x[316]+x[334]+x[352]+x[370]+x[388]+x[406]+x[424]+x[442]+x[460])^2/(x[28]*x[28])+0.35*x[262]+0.35*x[280]+0.35*x[298]+0.35*x[316]+0.35*x[334]+0.35*x[352]+0.35*x[370]+0.35*x[388]+0.35*x[406]+0.35*x[424]+0.35*x[442]+0.35*x[460]+0.35*sqrt(x[10])+0.35*sqrt(x[28])+0.55* (x[47]+x[65]+x[83]+x[101]+x[119]+x[137]+x[155]+x[173]+x[191]+x[209]+x[227]+x[245])^2/(x[11]*x[11])+0.55*x[47]+0.55*x[65]+0.55*x[83]+0.55*x[101]+0.55*x[119]+0.55*x[137]+0.55*x[155]+0.55*x[173]+0.55*x[191]+0.55*x[209]+0.55*x[227]+0.55*x[245]+0.55* (x[263]+x[281]+x[299]+x[317]+x[335]+x[353]+x[371]+x[389]+x[407]+x[425]+x[443]+x[461])^2/(x[29]*x[29])+0.55*x[263]+0.55*x[281]+0.55*x[299]+0.55*x[317]+0.55*x[335]+0.55*x[353]+0.55*x[371]+0.55*x[389]+0.55*x[407]+0.55*x[425]+0.55*x[443]+0.55*x[461]+0.55*sqrt(x[11])+0.55*sqrt(x[29])+0.15* (x[48]+x[66]+x[84]+x[102]+x[120]+x[138]+x[156]+x[174]+x[192]+x[210]+x[228]+x[246])^2/(x[12]*x[12])+0.15*x[48]+0.15*x[66]+0.15*x[84]+0.15*x[102]+0.15*x[120]+0.15*x[138]+0.15*x[156]+0.15*x[174]+0.15*x[192]+0.15*x[210]+0.15*x[228]+0.15*x[246]+0.15* (x[264]+x[282]+x[300]+x[318]+x[336]+x[354]+x[372]+x[390]+x[408]+x[426]+x[444]+x[462])^2/(x[30]*x[30])+0.15*x[264]+0.15*x[282]+0.15*x[300]+0.15*x[318]+0.15*x[336]+0.15*x[354]+0.15*x[372]+0.15*x[390]+0.15*x[408]+0.15*x[426]+0.15*x[444]+0.15*x[462]+0.15*sqrt(x[12])+0.15*sqrt(x[30])+0.4* (x[49]+x[67]+x[85]+x[103]+x[121]+x[139]+x[157]+x[175]+x[193]+x[211]+x[229]+x[247])^2/(x[13]*x[13])+0.4*x[49]+0.4*x[67]+0.4*x[85]+0.4*x[103]+0.4*x[121]+0.4*x[139]+0.4*x[157]+0.4*x[175]+0.4*x[193]+0.4*x[211]+0.4*x[229]+0.4*x[247]+0.4* (x[265]+x[283]+x[301]+x[319]+x[337]+x[355]+x[373]+x[391]+x[409]+x[427]+x[445]+x[463])^2/(x[31]*x[31])+0.4*x[265]+0.4*x[283]+0.4*x[301]+0.4*x[319]+0.4*x[337]+0.4*x[355]+0.4*x[373]+0.4*x[391]+0.4*x[409]+0.4*x[427]+0.4*x[445]+0.4*x[463]+0.4*sqrt(x[13])+0.4*sqrt(x[31])+0.6* (x[50]+x[68]+x[86]+x[104]+x[122]+x[140]+x[158]+x[176]+x[194]+x[212]+x[230]+x[248])^2/(x[14]*x[14])+0.6*x[50]+0.6*x[68]+0.6*x[86]+0.6*x[104]+0.6*x[122]+0.6*x[140]+0.6*x[158]+0.6*x[176]+0.6*x[194]+0.6*x[212]+0.6*x[230]+0.6*x[248]+0.6* (x[266]+x[284]+x[302]+x[320]+x[338]+x[356]+x[374]+x[392]+x[410]+x[428]+x[446]+x[464])^2/(x[32]*x[32])+0.6*x[266]+0.6*x[284]+0.6*x[302]+0.6*x[320]+0.6*x[338]+0.6*x[356]+0.6*x[374]+0.6*x[392]+0.6*x[410]+0.6*x[428]+0.6*x[446]+0.6*x[464]+0.6*sqrt(x[14])+0.6*sqrt(x[32])+0.25* (x[51]+x[69]+x[87]+x[105]+x[123]+x[141]+x[159]+x[177]+x[195]+x[213]+x[231]+x[249])^2/(x[15]*x[15])+0.25*x[51]+0.25*x[69]+0.25*x[87]+0.25*x[105]+0.25*x[123]+0.25*x[141]+0.25*x[159]+0.25*x[177]+0.25*x[195]+0.25*x[213]+0.25*x[231]+0.25*x[249]+0.25* (x[267]+x[285]+x[303]+x[321]+x[339]+x[357]+x[375]+x[393]+x[411]+x[429]+x[447]+x[465])^2/(x[33]*x[33])+0.25*x[267]+0.25*x[285]+0.25*x[303]+0.25*x[321]+0.25*x[339]+0.25*x[357]+0.25*x[375]+0.25*x[393]+0.25*x[411]+0.25*x[429]+0.25*x[447]+0.25*x[465]+0.25*sqrt(x[15])+0.25*sqrt(x[33])+0.3* (x[52]+x[70]+x[88]+x[106]+x[124]+x[142]+x[160]+x[178]+x[196]+x[214]+x[232]+x[250])^2/(x[16]*x[16])+0.3*x[52]+0.3*x[70]+0.3*x[88]+0.3*x[106]+0.3*x[124]+0.3*x[142]+0.3*x[160]+0.3*x[178]+0.3*x[196]+0.3*x[214]+0.3*x[232]+0.3*x[250]+0.3* (x[268]+x[286]+x[304]+x[322]+x[340]+x[358]+x[376]+x[394]+x[412]+x[430]+x[448]+x[466])^2/(x[34]*x[34])+0.3*x[268]+0.3*x[286]+0.3*x[304]+0.3*x[322]+0.3*x[340]+0.3*x[358]+0.3*x[376]+0.3*x[394]+0.3*x[412]+0.3*x[430]+0.3*x[448]+0.3*x[466]+0.3*sqrt(x[16])+0.3*sqrt(x[34])+0.5* (x[53]+x[71]+x[89]+x[107]+x[125]+x[143]+x[161]+x[179]+x[197]+x[215]+x[233]+x[251])^2/(x[17]*x[17])+0.5*x[53]+0.5*x[71]+0.5*x[89]+0.5*x[107]+0.5*x[125]+0.5*x[143]+0.5*x[161]+0.5*x[179]+0.5*x[197]+0.5*x[215]+0.5*x[233]+0.5*x[251]+0.5* (x[269]+x[287]+x[305]+x[323]+x[341]+x[359]+x[377]+x[395]+x[413]+x[431]+x[449]+x[467])^2/(x[35]*x[35])+0.5*x[269]+0.5*x[287]+0.5*x[305]+0.5*x[323]+0.5*x[341]+0.5*x[359]+0.5*x[377]+0.5*x[395]+0.5*x[413]+0.5*x[431]+0.5*x[449]+0.5*x[467]+0.5*sqrt(x[17])+0.5*sqrt(x[35])+0.5* (x[54]+x[72]+x[90]+x[108]+x[126]+x[144]+x[162]+x[180]+x[198]+x[216]+x[234]+x[252])^2/(x[18]*x[18])+0.5*x[54]+0.5*x[72]+0.5*x[90]+0.5*x[108]+0.5*x[126]+0.5*x[144]+0.5*x[162]+0.5*x[180]+0.5*x[198]+0.5*x[216]+0.5*x[234]+0.5*x[252]+0.5* (x[270]+x[288]+x[306]+x[324]+x[342]+x[360]+x[378]+x[396]+x[414]+x[432]+x[450]+x[468])^2/(x[36]*x[36])+0.5*x[270]+0.5*x[288]+0.5*x[306]+0.5*x[324]+0.5*x[342]+0.5*x[360]+0.5*x[378]+0.5*x[396]+0.5*x[414]+0.5*x[432]+0.5*x[450]+0.5*x[468]+0.5*sqrt(x[18])+0.5*sqrt(x[36]))+objvar == 0.0)
