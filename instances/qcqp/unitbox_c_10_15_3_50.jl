...
# ----- Constraints ----- #
@NLconstraint(m, e1, -(-1.09*x[2]*x[7]-0.51*x[2]*x[8]+0.69*x[2]*x[9]+1.91*x[2]*x[10]-1.35*x[2]*x[11]+1.18*x[3]*x[7]+0.44*x[3]*x[8]-0.15*x[3]*x[9]+0.49*x[3]*x[10]-1.04*x[3]*x[11]+1.5*x[4]*x[7]+0.62*x[4]*x[8]+0.47*x[4]*x[9]+0.98*x[4]*x[10]+1.23*x[4]*x[11]+1.13*x[5]*x[7]-1.31*x[5]*x[8]-1.09*x[5]*x[9]-0.9*x[5]*x[10]+0.19*x[5]*x[11]+0.77*x[6]*x[7]-1.62*x[6]*x[8]-1.03*x[6]*x[9]-0.56*x[6]*x[10]+0.94*x[6]*x[11]-0.87*x[2]-0.16*x[3]+0.48*x[4]+0.12*x[5]-0.09*x[6]-0.47*x[7]-0.94*x[8]-0.16*x[9]+0.58*x[10]+0.11*x[11])+x[1] == 0.0)
...
@NLconstraint(m, e12, (-0.65*x[2]*x[7])-1.19*x[2]*x[8]-1.24*x[2]*x[9]-1.17*x[2]*x[10]-1.45*x[2]*x[11]+2*x[3]*x[7]-0.41*x[3]*x[8]+1.99*x[3]*x[9]-0.84*x[3]*x[10]-1.9*x[3]*x[11]+0.86*x[4]*x[7]-0.61*x[4]*x[8]-1.37*x[4]*x[9]+1.28*x[4]*x[10]+1.46*x[4]*x[11]+0.5*x[5]*x[8]-0.27*x[5]*x[7]+1.47*x[5]*x[9]-1.84*x[5]*x[10]-0.99*x[5]*x[11]+(-0.26*x[6]*x[7])-0.94*x[6]*x[8]-0.79*x[6]*x[9]-1.7*x[6]*x[10]-0.76*x[6]*x[11]+0.35*x[2]-0.01*x[3]+0.44*x[4]+0.79*x[5]+0.85*x[6]+0.2*x[7]+0.14*x[8]+0.08*x[9]-0.15*x[10]+0.42*x[11] <= 20.42)
@NLconstraint(m, e13, 0.62*x[2]*x[8]-1.2*x[2]*x[7]+1.69*x[2]*x[9]-1.47*x[2]*x[10]-1.46*x[2]*x[11]+(-1.52*x[3]*x[7])-1.95*x[3]*x[8]-0.98*x[3]*x[9]+0.64*x[3]*x[10]-0.01*x[3]*x[11]+0.67*x[4]*x[7]-0.79*x[4]*x[8]+1.21*x[4]*x[9]+1.06*x[4]*x[10]-0.27*x[4]*x[11]+(-0.76*x[5]*x[7])-0.91*x[5]*x[8]-0.85*x[5]*x[9]-0.07*x[5]*x[10]+1.77*x[5]*x[11]+1.72*x[6]*x[8]-0.84*x[6]*x[7]+0.38*x[6]*x[9]-1.44*x[6]*x[10]+0.71*x[6]*x[11]-0.89*x[2]+0.31*x[3]-0.83*x[4]-0.68*x[5]-0.72*x[6]-0.08*x[7]+0.79*x[8]-0.43*x[9]-0.08*x[10]-0.53*x[11] <= 39.51)
@NLconstraint(m, e14, 1.23*x[2]*x[8]-1.8*x[2]*x[7]+1.47*x[2]*x[9]+1.67*x[2]*x[10]-0.42*x[2]*x[11]+(-1.67*x[3]*x[7])-1.76*x[3]*x[8]-0.76*x[3]*x[9]-1.77*x[3]*x[10]+0.56*x[3]*x[11]+0.98*x[4]*x[7]-0.27*x[4]*x[8]-1.18*x[4]*x[9]-1.91*x[4]*x[10]+1.16*x[4]*x[11]+1.75*x[5]*x[7]-1.35*x[5]*x[8]-1.4*x[5]*x[9]-0.47*x[5]*x[10]+0.88*x[5]*x[11]+0.77*x[6]*x[8]-1.94*x[6]*x[7]+1.06*x[6]*x[9]-0.55*x[6]*x[10]-x[6]*x[11]-0.89*x[2]+0.51*x[3]+0.75*x[4]-0.04*x[5]+0.24*x[6]+0.65*x[7]+0.4*x[8]+0.31*x[9]+0.23*x[10]-0.45*x[11] <= 23.96)
@NLconstraint(m, e15, 0.22*x[2]*x[7]+0.35*x[2]*x[8]-1.19*x[2]*x[9]+1.29*x[2]*x[10]+1.59*x[2]*x[11]+(-1.25*x[3]*x[7])-0.63*x[3]*x[8]-1.34*x[3]*x[9]+0.16*x[3]*x[10]+1.2*x[3]*x[11]+1.41*x[4]*x[7]+1.89*x[4]*x[8]+0.91*x[4]*x[9]-1.47*x[4]*x[10]+0.71*x[4]*x[11]+(-0.11*x[5]*x[7])-1.01*x[5]*x[8]-0.86*x[5]*x[9]-1.63*x[5]*x[10]-0.58*x[5]*x[11]+0.29*x[6]*x[7]+0.32*x[6]*x[8]+1.55*x[6]*x[9]-1.9*x[6]*x[10]-1.09*x[6]*x[11]-0.59*x[2]-0.94*x[3]-0.33*x[4]+0.25*x[5]-0.82*x[6]-0.74*x[7]+0.26*x[8]+0.16*x[9]+0.74*x[10]+0.15*x[11] <= 40.95)
@NLconstraint(m, e16, 1.64*x[2]*x[7]-0.77*x[2]*x[8]-1.67*x[2]*x[9]-0.07*x[2]*x[10]+0.77*x[2]*x[11]+1.95*x[3]*x[7]+1.65*x[3]*x[8]-0.95*x[3]*x[9]-0.11*x[3]*x[10]+1.62*x[3]*x[11]+(-0.68*x[4]*x[7])-0.19*x[4]*x[8]-0.72*x[4]*x[9]-0.09*x[4]*x[10]+1.28*x[4]*x[11]+(-1.17*x[5]*x[7])-0.42*x[5]*x[8]-1.89*x[5]*x[9]-1.62*x[5]*x[10]-1.34*x[5]*x[11]+0.66*x[6]*x[7]+0.75*x[6]*x[8]-0.02*x[6]*x[9]-0.66*x[6]*x[10]+0.82*x[6]*x[11]+0.75*x[2]-0.02*x[3]-0.33*x[4]+0.76*x[5]+0.34*x[6]+0.46*x[7]-0.62*x[8]-0.76*x[9]-0.67*x[10]+0.87*x[11] <= 70.7)
